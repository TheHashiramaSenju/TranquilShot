...
def generate_launch_description():
    ...
    def make_node(pkg, exe, params_file):
        return Node(package=pkg, executable=exe, name=exe,
                    output='screen', parameters=[{'use_sim_time': False}, params_file])

    return LaunchDescription([
        Node(package='tranq_drivers', executable='camera_driver',
             name='camera_driver', output='screen'),
        Node(package='tranq_drivers', executable='vl53l1x_driver',
             name='vl53l1x_driver', output='screen'),
        Node(package='tranq_drivers', executable='optical_flow_node',
             name='optical_flow_node', output='screen'),
